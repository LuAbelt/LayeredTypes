    ...
    class StateLayer(lark.visitors.Interpreter):
        class ArgumentState:
            ...
            def __init__(self, state: str):
                STATES_REGEX = r"\{([a-zA-Z\s]*(=>[a-zA-Z\s]*)?(,[a-zA-Z\s]*(=>[a-zA-Z\s]*)?)*)\}"
                ...
                for part in state_parts:
                    if "=>" in part:
                        ...
                        self.required_states.add(part.strip())
            ...
            def apply_state_transitions(self, states):
                for before, after in self.state_transitions:
                    if not before or before in states:
                        states.remove(before) if before in states else None
                        states.add(after) if after else None
        def __init__(self):
        ...
        def layer(self, tree):
            ...
            if len(states) == 1:
                ...
                if not arg_state.check_required_states(self.__states[identifier]):
                    raise TypeError(f"{tree.meta.line}:{tree.meta.column}: Identifier {identifier} does not have the required state.")
        ...
        def fun_call(self, tree):
            ...
            for i, arg in enumerate(tree.children[1:-1]):
                if not self.__function_states[fun_identifier][i].check_required_states(self.visit(arg)):
                    raise TypeError(f"{tree.meta.line}:{tree.meta.column}: Function {fun_identifier} was called with invalid state for argument {i}.")
